...
test.describe('Model Benchmark', () => {
  ...
  for (const model of MODELS) {
    test(`Benchmark ${model}`, async ({ page }) => {
      ...
      console.log(
        `Finished ${model}: WER=${(results.avgWER * 100).toFixed(1)}%, CEFR=${(results.cefrAccuracy * 100).toFixed(0)}%, Speed=${results.avgTimePerFile.toFixed(1)}s`
      );
    });
  }
...
async function runBenchmark(page: any, modelId: string, limit: number) {
  ...
  const avgClarity =
    results.reduce((acc: number, r: any) => acc + (r.clarityScore || 0), 0) / results.length;
