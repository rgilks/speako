...
training_image = image.add_local_dir(
    "test-data/reference-materials/stms", remote_path="/stm-data"
).add_local_file("ml/cefr_utils.py", remote_path="/root/cefr_utils.py")


@app.function(image=training_image, gpu="T4", timeout=7200, volumes={"/models": volume})
def train_deberta(
    ...
    import evaluate
    from cefr_utils import (
        parse_stm_file,
        load_universal_cefr,
        LABEL2ID,
        ID2LABEL,
        augment_text_with_noise,
    )
    ...
    ort_model = ORTModelForSequenceClassification.from_pretrained(
        f"{OUTPUT_DIR}/final", export=True
    )
    ...
    try:
        ...
        if os.path.exists(stm_test_path):
            ...
            print(
                f"STM Test Results: F1={res['eval_f1']:.4f}, Acc={res['eval_accuracy']:.4f}"
            )
