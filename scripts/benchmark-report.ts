...
function generateReport() {
  if (!existsSync(INPUT_FILE)) {
    console.error(`❌ No validation results found. Run: npm run validate`);
    process.exit(1);
  }
...
`;
  ...
  for (const [level, stats] of levels) {
    const bar = '█'.repeat(Math.round(stats.avgWER * 20));
...
`;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const bar = '█'.repeat(Math.round((histogram[i] / maxHist) * 40));
