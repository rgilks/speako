...
function TableRow({ result: r, isSelected, onSelect }: TableRowProps) {
  return (
    <tr
      onClick={() => onSelect(r)}
      style={{
    ...
    >
      <td
        style={{
          padding: '6px',
          color: r.wer < 0.2 ? '#4ade80' : r.wer < 0.4 ? '#fbbf24' : '#f87171',
          fontWeight: 'bold',
        }}
      >
        {(r.wer * 100).toFixed(0)}%
      ...
      <td style={{ padding: '6px' }}>
        <span style={{ color: r.cefrMatch ? '#4ade80' : '#f87171' }}>{r.detectedCEFR}</span>
        <span style={{ color: 'var(--text-tertiary)', marginLeft: '4px' }}>({r.labeledCEFR})</span>
      </td>
        ...
        <tbody>
          {results.map((r) => (
            <TableRow
              key={r.fileId}
              result={r}
              isSelected={selectedFileId === r.fileId}
