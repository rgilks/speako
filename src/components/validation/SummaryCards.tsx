...
export function SummaryCards({ avgWER, cefrAccuracy, avgClarity, totalFiles }: SummaryCardsProps) {
  const cardStyle = (condition: 'good' | 'warning' | 'bad') => ({
    background: condition === 'good' ? 'rgba(34, 197, 94, 0.1)' 
              : condition === 'warning' ? 'rgba(245, 158, 11, 0.1)' 
              : 'rgba(239, 68, 68, 0.1)',
    padding: '1rem',
    borderRadius: '8px',
  ...
  const getAccuracyCondition = (acc: number) => acc > 0.6 ? 'good' : acc > 0.4 ? 'warning' : 'bad';
  const getClarityCondition = (clarity: number) => clarity > 80 ? 'good' : clarity > 60 ? 'warning' : 'bad';
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
      ...
      <div style={cardStyle(getAccuracyCondition(cefrAccuracy))}>
        <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{(cefrAccuracy * 100).toFixed(0)}%</div>
