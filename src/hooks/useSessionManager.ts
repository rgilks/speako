...
export function useSessionManager() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      console.log(
        '[SessionManager] Using LocalTranscriber.',
        selectedDeviceId.value ? `Device: ${selectedDeviceId.value}` : '(default)'
      );
      ...
      if (!isCEFRClassifierReady()) {
        loadCEFRClassifier()
          ...
          .catch((e) =>
            console.warn('[SessionManager] CEFR classifier unavailable, will use heuristic', e)
          );
      }
    } catch (e) {
  ...
  const handleStop = async () => {
    ...
    if (
      ...
      result.text.includes('[BLANK_AUDIO]')
    ) {
    ...
    try {
      ...
      console.log(
        `[SessionManager] Metrics calculated (${metricsResult.cefr_method}):`,
        metricsResult
      );
      ...
      metrics.value = {
        ...
        cefr_description:
          metricsResult.cefr_method === 'ml'
            ? `${(metricsResult.cefr_confidence! * 100).toFixed(0)}% confidence`
            : 'heuristic',
        fluency_score: 0,
      ...
      console.log(
        `[SessionManager] Processing complete in ${Math.round(Date.now() - startTime.current)}ms`
      );
